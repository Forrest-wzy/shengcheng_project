...
class lightGenerator(nn.Module):
    def __init__(self):
        ...
        self.layer1 = self._make_layer(ResidualBlock, 16, 3, stride=1)  # 256×256
        self.layer2 = self._make_layer(ResidualBlock, 32, 3, stride=2)  # 128×128
        self.layer3 = self._make_layer(ResidualBlock, 64, 3, stride=2)  # 64×64
        self.layer4 = self._make_layer(ResidualBlock, 128, 3, stride=2)  # 32x32
        self.bottleneck = self._make_layer(ResidualBlock, 128, 3, stride=1)
        ...
        self.decoder2 = nn.Sequential(
            nn.ConvTranspose2d(128, 32, 4, 2, 1),  # 输入 64 通道（拼接后）
            nn.InstanceNorm2d(32),
            nn.ReLU(inplace=True)
        )

        self.decoder3 = nn.Sequential(
            nn.ConvTranspose2d(64, 16, 4, 2, 1),  # 输入 32 通道（拼接后）
            nn.InstanceNorm2d(16),
            nn.ReLU(inplace=True)
        )

        self.decoder4 = nn.Sequential(
            nn.Conv2d(32, 3, 3, 1, 1),  # (32+32拼接)
            nn.Tanh()
        )
...
start_epoch =37
generator.load_state_dict(torch.load(f'edges2shoes_generator128_epoch_{start_epoch}.pth', map_location=device))
discriminator.load_state_dict(torch.load(f'edges2shoes_discriminator128_epoch_{start_epoch}.pth', map_location=device))
optimizer_G.load_state_dict(torch.load(f'edges2shoes_optimizer_G128_epoch_{start_epoch}.pth',map_location=device))
optimizer_D.load_state_dict(torch.load(f'edges2shoes_optimizer_D128_epoch_{start_epoch}.pth',map_location=device))
#训练
import torch
...
for epoch in range(start_epoch+1,epochs):
    ...
    if epoch % 1 == 0:
        # 如果是预热阶段，只打印 G_loss
         ...
         torch.save(generator.state_dict(), f'edges2shoes_generator128_epoch_{epoch}.pth')
         torch.save(discriminator.state_dict(), f'edges2shoes_discriminator128_epoch_{epoch}.pth')
         torch.save(optimizer_G.state_dict(), f'edges2shoes_optimizer_G128_epoch_{epoch}.pth')
         torch.save(optimizer_D.state_dict(), f'edges2shoes_optimizer_D128_epoch_{epoch}.pth')

    generator.eval()
    os.makedirs('generated_shoes128', exist_ok=True)
    with torch.no_grad():
        ...
        img.save(f'generated_shoes128/epoch_{epoch}_fake.png')
    generator.train()
    ...
    with torch.no_grad():
        for line, photo,filename in test_loader:
            ...
            for i in range(fake_photo.size(0)):
                ...
                img.save(f'generated_shoes128/{filename[i]}')
                count += 1
    print(f"已生成 {count} 张假图片到 generated_shoes128/")
